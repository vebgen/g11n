...
import { interpolateName, MessageDescriptor } from '@formatjs/ts-transformer';
...
export function isValidIdentifier(k: string): boolean {
    try {
...
export function messageASTToTSNode(
    factory: typescript.NodeFactory,
...
export interface Opts {
    ...
    /**
     * Callback function that gets called every time we encountered something
...
export function isMultipleMessageDecl(
    ts: TypeScript,
...
export function evaluateStringConcat(
    ts: TypeScript,
        ...
        if (name && ts.isIdentifier(name) && initializer) {
            ...
            // {id: `id`}
                ...
                // <FormattedMessage foo={'bar' + 'baz'} />
                    ...
                    const { expression } = initializer;
                    const [result, isStatic] = evaluateStringConcat(
                        ts,
                        expression,
                    );
                    if (isStatic) {
 ...
 * @param sf
 */
export function isMemberMethodFormatMessageCall(
    ts: TypeScript,
    node: typescript.CallExpression,
    additionalFunctionNames: string[],
) {
...
export function setAttributesInObject(
    ts: TypeScript,
    factory: typescript.NodeFactory,
    ...
    for (const prop of node.properties) {
        if (
            ...
            MESSAGE_DESC_KEYS.includes(
                prop.name.text as keyof MessageDescriptor,
            )
        ) {
            continue;
        }
...
export function generateNewProperties(
    ts: TypeScript,
    factory: typescript.NodeFactory,
